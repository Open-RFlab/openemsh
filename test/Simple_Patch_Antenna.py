...
# -*- coding: utf-8 -*-
"""
 https://github.com/thliebig/openEMS/raw/refs/heads/master/python/Tutorials/Simple_Patch_Antenna.py

 Simple Patch Antenna Tutorial adapted for OpenEMSH
...
from openemsh import *

### General parameter setup
post_proc_only = False
use_oemsh = False
if len(sys.argv) > 1 and sys.argv[1] == "openemsh":
	use_oemsh = True
print(f"Running in { "OpenEMSH" if use_oemsh else "legacy" } mode\n")
Sim_Path = os.path.join(tempfile.gettempdir(), 'Simp_Patch')
CSX_file = os.path.join(Sim_Path, 'simp_patch.xml')
ensure_sim_path(Sim_Path, cleanup=True)
...
FDTD2 = openEMS(NrTS=30000, EndCriteria=1e-4)
...
### Mesh generation
if use_oemsh:
	run_openemsh(FDTD, CSX_file, ["-Gvf", "--integrate-old-mesh=true", "--dmax=5"])
else:
