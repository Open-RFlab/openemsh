	...
	GIVEN("A conflict manager and some edges") {
		...
		WHEN("Two vertical edges that are colinear are reported as colinear") {
			...
			THEN("A COLINEAR_EDGES conflict should be registered") {
				REQUIRE(cm.get_current_state().all_colinear_edges[X].size() == 1);
				REQUIRE(cm.get_current_state().all_colinear_edges[X][0]->kind == Conflict::Kind::COLINEAR_EDGES);
				ConflictColinearEdges* conflict = cm.get_current_state().all_colinear_edges[X][0].get();
				REQUIRE(conflict->get_current_state().edges.size() == 2);
				REQUIRE(conflict->get_current_state().edges[0] == &a);
				REQUIRE(conflict->get_current_state().edges[1] == &b);
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(b.get_current_state().conflicts.size() == 1);
				REQUIRE(b.get_current_state().conflicts[0] == conflict);
			}
			...
			AND_WHEN("A third vertical colinear edge is reported as colinear") {
				...
				THEN("It should be added to the existing conflict") {
					...
					REQUIRE(cm.get_current_state().all_colinear_edges[X][0]->kind == Conflict::Kind::COLINEAR_EDGES);
					ConflictColinearEdges* conflict = cm.get_current_state().all_colinear_edges[X][0].get();
					REQUIRE(conflict->get_current_state().edges.size() == 3);
					REQUIRE(conflict->get_current_state().edges[2] == &c);
					REQUIRE(c.get_current_state().conflicts.size() == 1);
					REQUIRE(c.get_current_state().conflicts[0] == conflict);
				}
				THEN("It should be added to the existing conflict") {
			...
			AND_WHEN("Two other colinear edges elsewhere are reported as colinear") {
				...
				THEN("A new COLINEAR_EDGES conflict should be registered") {
					REQUIRE(cm.get_current_state().all_colinear_edges[X].size() == 2);
					REQUIRE(cm.get_current_state().all_colinear_edges[X][1]->kind == Conflict::Kind::COLINEAR_EDGES);
					ConflictColinearEdges* conflict = cm.get_current_state().all_colinear_edges[X][1].get();
					REQUIRE(conflict->get_current_state().edges.size() == 2);
					REQUIRE(conflict->get_current_state().edges[0] == &d);
					REQUIRE(conflict->get_current_state().edges[1] == &e);
				}
			}
		}
		...
		WHEN("Two horizontal edges that are colinear are reported as colinear") {
			...
			cm.add_colinear_edges(&a, &b);
			THEN("A COLINEAR_EDGES conflict should be registered") {
				REQUIRE(cm.get_current_state().all_colinear_edges[Y].size() == 1);
				REQUIRE(cm.get_current_state().all_colinear_edges[Y][0]->kind == Conflict::Kind::COLINEAR_EDGES);
				ConflictColinearEdges* conflict = cm.get_current_state().all_colinear_edges[Y][0].get();
				REQUIRE(conflict->get_current_state().edges.size() == 2);
				REQUIRE(conflict->get_current_state().edges[0] == &a);
				REQUIRE(conflict->get_current_state().edges[1] == &b);
				REQUIRE(a.get_current_state().conflicts.size() == 1);
		...
		WHEN("Two diagonal edges that are colinear are reported as colinear") {
			...
			THEN("No conflict should be registered") {
				REQUIRE_FALSE(cm.get_current_state().all_colinear_edges[X].size());
				REQUIRE_FALSE(cm.get_current_state().all_colinear_edges[Y].size());
				REQUIRE_FALSE(cm.get_current_state().all_colinear_edges[Z].size());
			}
		}
...
SCENARIO("void ConflictManager::add_edge_in_polygon(Edge* a, Polygon* polygon, Range const range, std::optional<Edge const*> b)", "[conflict_manager]") {
	...
	GIVEN("A conflict manager, an edge and some polygons") {
		ConflictManager cm(t);
		Polygon p(XY, material, "", 0, { 0, 0 }, from_init_list<Point>({
			{ 3, 1 }, { 6, 1 }, { 6, 6 }, { 1, 6 }, { 1, 3 },
			{ 2, 3 }, { 2, 5 }, { 5, 5 }, { 5, 2 }, { 3, 2 }}), t);
		Polygon q(XY, material, "", 0, { 0, 0 }, from_init_list<Point>({{ 0, 0 }, { 0, 8 }, { 8, 8 }, { 8, 0 }}), t);
		WHEN("A vertical edge that is in a polygon is reported as partially in this polygon") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				REQUIRE(cm.get_current_state().all_edge_in_polygons[XY].size() == 1);
				REQUIRE(cm.get_current_state().all_edge_in_polygons[XY][0]->kind == Conflict::Kind::EDGE_IN_POLYGON);
				ConflictEdgeInPolygon* conflict = cm.get_current_state().all_edge_in_polygons[XY][0].get();
				REQUIRE(conflict->edge == &a);
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 4, 1 }, { 4, 2 }));
				REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
				REQUIRE(p.get_current_state().conflicts[0] == conflict);
			}
			AND_WHEN("The same conflict is reported another time") {
				cm.add_edge_in_polygon(&a, &p, Range({ 4, 1 }, { 4, 2 }));
				THEN("No conflict should be added nor modified") {
			...
			AND_WHEN("The edge is reported as also partially in the same polygon but by another overlap range") {
				...
				THEN("It should be added to the existing conflict") {
					...
					REQUIRE(cm.get_current_state().all_edge_in_polygons[XY][0]->kind == Conflict::Kind::EDGE_IN_POLYGON);
					ConflictEdgeInPolygon* conflict = cm.get_current_state().all_edge_in_polygons[XY][0].get();
					REQUIRE(conflict->get_current_state().overlaps.size() == 2);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[1]) == &p);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[1]) == Range({ 4, 5 }, { 4, 6 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[1]));
					REQUIRE(a.get_current_state().conflicts.size() == 1);
					REQUIRE(p.get_current_state().conflicts.size() == 1);
				}
			}
			AND_WHEN("An other edge is reported as totally in the polygon") {
				...
				THEN("An other EDGE_IN_POLYGON conflict should be registered") {
					REQUIRE(cm.get_current_state().all_edge_in_polygons[XY].size() == 2);
					REQUIRE(cm.get_current_state().all_edge_in_polygons[XY][1]->kind == Conflict::Kind::EDGE_IN_POLYGON);
					ConflictEdgeInPolygon* conflict = cm.get_current_state().all_edge_in_polygons[XY][1].get();
					REQUIRE(conflict->edge == &b);
					REQUIRE(conflict->get_current_state().overlaps.size() == 1);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 5, 5 }, { 5, 6 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
					REQUIRE(b.get_current_state().conflicts.size() == 1);
					REQUIRE(b.get_current_state().conflicts[0] == conflict);
					REQUIRE(p.get_current_state().conflicts.size() == 2);
					REQUIRE(p.get_current_state().conflicts[1] == conflict);
				}
			}
			AND_WHEN("The edge is reported as also totally in a second polygon") {
				...
				THEN("It should be added to the existing conflict") {
					...
					ConflictEdgeInPolygon* conflict = cm.get_current_state().all_edge_in_polygons[XY][0].get();
					REQUIRE(conflict->get_current_state().overlaps.size() == 2);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[1]) == &q);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[1]) == Range({ 4, 1 }, { 4, 7 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[1]));
					REQUIRE(a.get_current_state().conflicts.size() == 1);
					REQUIRE(p.get_current_state().conflicts.size() == 1);
				}
		...
		WHEN("An horizontal edge that is in a polygon is reported as partially in this polygon") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 1, 4 }, { 2, 4 }));
				REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
			...
			AND_WHEN("The edge is reported as also partially in the same polygon but by another overlap range") {
				...
				THEN("It should be added to the existing conflict") {
					...
					REQUIRE(conflict->get_current_state().overlaps.size() == 2);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[1]) == &p);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[1]) == Range({ 5, 4 }, { 6, 4 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[1]));
					REQUIRE(a.get_current_state().conflicts.size() == 1);
					REQUIRE(p.get_current_state().conflicts.size() == 1);
				}
			...
			AND_WHEN("An other edge is reported as totally in the polygon") {
				...
				THEN("An other EDGE_IN_POLYGON conflict should be registered") {
					...
					REQUIRE(conflict->get_current_state().overlaps.size() == 1);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 5, 5 }, { 6, 5 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
					REQUIRE(b.get_current_state().conflicts.size() == 1);
					REQUIRE(b.get_current_state().conflicts[0] == conflict);
					REQUIRE(p.get_current_state().conflicts.size() == 2);
			...
			AND_WHEN("The edge is reported as also totally in a second polygon") {
				...
				THEN("It should be added to the existing conflict") {
					...
					REQUIRE(conflict->get_current_state().overlaps.size() == 2);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[1]) == &q);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[1]) == Range({ 1, 4 }, { 7, 4 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[1]));
					REQUIRE(a.get_current_state().conflicts.size() == 1);
					REQUIRE(p.get_current_state().conflicts.size() == 1);
				}
		...
		WHEN("A diagonal edge that is in a polygon is reported as partially in this polygon") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 3, 6 }, { 4, 5 }));
				REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
			...
			AND_WHEN("The edge is reported as also partially in the same polygon but by another overlap range") {
				...
				THEN("It should be added to the existing conflict") {
					...
					REQUIRE(conflict->get_current_state().overlaps.size() == 2);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[1]) == &p);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[1]) == Range({ 5, 4 }, { 6, 3 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[1]));
					REQUIRE(a.get_current_state().conflicts.size() == 1);
					REQUIRE(p.get_current_state().conflicts.size() == 1);
				}
			...
			AND_WHEN("An other edge is reported as totally in the polygon") {
				...
				THEN("An other EDGE_IN_POLYGON conflict should be registered") {
					...
					REQUIRE(conflict->get_current_state().overlaps.size() == 1);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 5, 5 }, { 6, 6 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
					REQUIRE(b.get_current_state().conflicts.size() == 1);
					REQUIRE(b.get_current_state().conflicts[0] == conflict);
					REQUIRE(p.get_current_state().conflicts.size() == 2);
			...
			AND_WHEN("The edge is reported as also totally in a second polygon") {
				...
				THEN("It should be added to the existing conflict") {
					...
					REQUIRE(conflict->get_current_state().overlaps.size() == 2);
					REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[1]) == &q);
					REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[1]) == Range({ 2, 7 }, { 7, 2 }));
					REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[1]));
					REQUIRE(a.get_current_state().conflicts.size() == 1);
					REQUIRE(p.get_current_state().conflicts.size() == 1);
				}
		...
		WHEN("A vertical edge that is not in a polygon is reported as in this polygon") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 8, 8 }, { 8, 9 }));
				REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
		...
		WHEN("An horizontal edge that is not in a polygon is reported as in this polygon") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 8, 8 }, { 9, 8 }));
				REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
		...
		WHEN("A diagonal edge that is not in a polygon is reported as in this polygon") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 8, 8 }, { 9, 9 }));
				REQUIRE_FALSE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
		...
		WHEN("A vertical edge that is on a polygon is reported as in this polygon and sharing an edge with it") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 3, 1 }, { 3, 2 }));
				REQUIRE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(std::get<EDGE>(conflict->get_current_state().overlaps[0]).value() == p.edges[0].get());
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
				REQUIRE(p.get_current_state().conflicts[0] == conflict);
		...
		WHEN("An horizontal edge that is on a polygon is reported as in this polygon and sharing an edge with it") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 1, 3 }, { 2, 3 }));
				REQUIRE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(std::get<EDGE>(conflict->get_current_state().overlaps[0]).value() == p.edges[5].get());
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
				REQUIRE(p.get_current_state().conflicts.size() == 1);
				REQUIRE(p.get_current_state().conflicts[0] == conflict);
		...
		WHEN("A diagonal edge that is not on a polygon is reported as in this polygon and sharing an edge with it") {
			...
			THEN("An EDGE_IN_POLYGON conflict should be registered") {
				...
				REQUIRE(conflict->get_current_state().overlaps.size() == 1);
				REQUIRE(std::get<POLYGON>(conflict->get_current_state().overlaps[0]) == &p);
				REQUIRE(std::get<RANGE>(conflict->get_current_state().overlaps[0]) == Range({ 1, 1 }, { 3, 3 }));
				REQUIRE(std::get<EDGE>(conflict->get_current_state().overlaps[0]));
				REQUIRE(std::get<EDGE>(conflict->get_current_state().overlaps[0]).value() == p.edges[0].get());
				REQUIRE(a.get_current_state().conflicts.size() == 1);
				REQUIRE(a.get_current_state().conflicts[0] == conflict);
		...
		WHEN("An edge is reported as being in a polygon from a different plane") {
			...
			THEN("No conflict should be added") {
				REQUIRE(cm.get_current_state().all_edge_in_polygons[YZ].size() == 0);
				REQUIRE(cm.get_current_state().all_edge_in_polygons[ZX].size() == 0);
				REQUIRE(cm.get_current_state().all_edge_in_polygons[XY].size() == 0);
			}
		}
	}
}
	...
	GIVEN("A conflict manager and some meshline policies") {
		...
		WHEN("Two meshline policies of same axis are reported") {
			...
			THEN("A TOO_CLOSE_MESHLINE_POLICIES conflict should be registered") {
				...
				REQUIRE_FALSE(a.get_current_state().is_enabled);
				REQUIRE_FALSE(b.get_current_state().is_enabled);
			}
			...
			AND_WHEN("Two other meshline policies of same axis are reported") {
				...
				THEN("A TOO_CLOSE_MESHLINE_POLICIES conflict should be registered") {
					...
					REQUIRE_FALSE(d.get_current_state().is_enabled);
					REQUIRE_FALSE(e.get_current_state().is_enabled);
				}
			}
